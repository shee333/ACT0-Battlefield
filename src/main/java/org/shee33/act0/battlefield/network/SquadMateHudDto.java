 ...
 */
public record SquadMateHudDto(String name, int healthPct, boolean alive, boolean self) {
    ...
    public static SquadMateHudDto decode(FriendlyByteBuf buf) {
        return new SquadMateHudDto(buf.readUtf(), buf.readVarInt(), buf.readBoolean(), buf.readBoolean());
    }
}
